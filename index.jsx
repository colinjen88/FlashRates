...
const CODE_EXAMPLES = {
  curl: `curl -X GET "https://goldlab.cloud/api/v1/latest" \\
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setActiveTab('dashboard')}>
            ...
            <span className="text-white font-bold text-xl tracking-tight">Goldlab<span className="text-emerald-500">.cloud</span></span>
          </div>
...
// 獨立的單一資產卡片組件，更新支援中文名稱
const AssetCard = ({ name, symbol, initialPrice, price, prevPrice, timestamp, source }) => {
  const isUp = price >= prevPrice;
  const colorClass = isUp ? 'text-emerald-400' : 'text-rose-400';
  const displayPrice = price || initialPrice;
  const change = price ? (price - initialPrice) : 0;
  const changePercent = initialPrice ? (change / initialPrice * 100) : 0;
    ...
    <div className="relative bg-slate-900 ring-1 ring-slate-800 rounded-xl p-5 shadow-2xl overflow-hidden flex flex-col h-full hover:ring-slate-700 transition-all">
      ...
      <div className="relative z-10">
        ...
        <div className="mb-4">
          ...
          <div className={`text-xs font-bold flex items-center ${colorClass} transition-colors duration-300`}>
        ...
        <div className="flex items-center justify-between gap-2 px-3 py-1.5 rounded-lg border bg-slate-900/50 border-slate-800 mb-4">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-bold text-slate-300 truncate max-w-[80px]">{source || '初始化中...'}</span>
          </div>
          <span className="text-[10px] text-emerald-500 font-mono">即時</span>
        </div>
        ...
        <div className="mt-auto">
          ...
          <div className="grid grid-cols-4 gap-1">
            {[...Array(8)].map((_, idx) => (
              <div key={idx} className={`h-0.5 rounded-full transition-all duration-500 ${idx < 5 ? 'bg-emerald-400' : 'bg-slate-700'}`}></div>
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="pt-24 pb-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-16">
        <AssetCard
          name="黃金現貨"
          symbol="XAU-USD"
          initialPrice={2650.50}
          price={marketData['XAU-USD']?.price}
          prevPrice={prevMarketData['XAU-USD'] || 2650.50}
          timestamp={marketData['XAU-USD']?.timestamp}
          source={marketData['XAU-USD']?.details?.[0]} // Just showing first source for now or 'details' field logic needs check
        />
        <AssetCard
          name="白銀現貨"
          symbol="XAG-USD"
          ...
          prevPrice={prevMarketData['XAG-USD'] || 31.45}
        ...
        <AssetCard
          ...
          prevPrice={prevMarketData['USD-TWD'] || 31.85}
        ...
        <div className="lg:col-span-9">
          <div className="prose prose-invert max-w-none">
            ...
            <p className="text-slate-400 text-lg mb-8">
              Goldlab.cloud API 支援同時請求多種貴金屬與法幣匯率數據。透過在參數中指定多個 <code>symbol</code>，
        ...
        <footer className="bg-slate-900 border-t border-slate-800 py-12 mt-12 relative z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-300 font-bold">Goldlab.cloud</span>
            </div>
